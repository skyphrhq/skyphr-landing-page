...
function OurProcessCard({ imageOptions, title, description, label }: OurProcessCardInterface) {
  return (
    <div className="w-full h-fit bg-(--about-us-card-bg) p-2.5 md:p-4 overflow-hidden rounded-lg md:rounded-xl">
      <div className="w-full flex flex-col items-start justify-start gap-4">
        <div className="w-full h-fit">
          <Image
            ...
            className={twMerge("rounded-md md:rounded-lg w-full object-cover object-center", imageOptions?.className)}
          />
        </div>
        <div className="w-full flex flex-col items-start justify-start gap-2.5 md:gap-5 px-2 md:px-4 pb-4">
          ...
          <span className="text-lg md:text-xl lg:text-2xl font-semibold text-(--text-main-color) font-instrument-sans">{title}</span>
